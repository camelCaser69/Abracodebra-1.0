...
namespace WegoSystem 
{
    public class GridPositionManager : MonoBehaviour 
    {
        ...
        // The private field that holds the grid reference.
        private Grid _tilemapGrid;

        // Public property to safely access the grid. It ensures the grid is synced if it hasn't been already.
        ...
        void Awake() {
            ...
            // Initial attempt to sync the grid. With Script Execution Order, this should always succeed.
            // If not, the lazy-loading property will handle it later.
            SyncWithTileGrid();
        }
        ...
        /// <summary>
        /// Attempts to find the TileInteractionManager's grid and link to it.
        /// </summary>
        public void SyncWithTileGrid() {
            if (TileInteractionManager.Instance != null && TileInteractionManager.Instance.interactionGrid != null) {
                ...
                if (debugMode && this._tilemapGrid != null)
                    Debug.Log($"[GridPositionManager] Successfully synced with TileInteractionManager's grid: '{this._tilemapGrid.name}'.");
            }
            else if (_tilemapGrid == null) { // Only log the error if we truly don't have a grid.
