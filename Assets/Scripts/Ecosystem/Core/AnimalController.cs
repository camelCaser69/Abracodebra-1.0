...
public class AnimalController : MonoBehaviour
{
    ...
    // --- MODIFIED: SeekFood with periodic reassessment ---
    void SeekFood() 
    { 
        ...
        if (shouldReassess || !targetValid) 
        {
            ...
            if (potentialBetterFood != null) 
            {
                ...
                if (!targetValid) 
                {
                    ...
                    if(Debug.isDebugBuild) Debug.Log($"[{gameObject.name} SeekFood] Found new food target (no previous): {potentialBetterFood.name}");
                }
                ...
                else if (potentialBetterFood != currentTargetFood) 
                {
                    ...
                    if (currentFoodItem != null && newFoodItem != null) 
                    {
                        ...
                        {
                            if(Debug.isDebugBuild) Debug.Log($"[{gameObject.name} SeekFood] Switching to higher priority food: {newFoodItem.foodType.foodName} (priority: {newPriority}) from {currentFoodItem.foodType.foodName} (priority: {currentPriority})");
                            currentTargetFood = potentialBetterFood;
                        }
                    }
                }
    ...
    { 
        ...
        { 
            ...
            sr.color = new Color(Mathf.Clamp01(c.r + Random.Range(-v, v)), Mathf.Clamp01(c.g + Random.Range(-v, v)), Mathf.Clamp01(c.b + Random.Range(-v, v)), c.a); 
        } 
    ...
    { 
        ...
        var entry = thoughtLibrary.allThoughts.FirstOrDefault(t => t != null && t.speciesName == SpeciesName && t.trigger == trigger); 
    ...
    private void EnsureHpTextReference() 
    { 
        if (hpText == null) 
        { 
            hpText = GetComponentInChildren<TextMeshProUGUI>(true); 
