...
namespace Abracodabra.UI.Toolkit
{
    public class UIHotbarController
    {
        ...
        // Events for hover (can be used for tooltips)
        public event Action<int, UIInventoryItem> OnSlotHoverEnter;
        ...
        void HandleSlotHoverEnter(int index)
        {
            ...
            if (hotbarItems != null && index >= 0 && index < hotbarItems.Count)
            {
                item = hotbarItems[index];
            }

            OnSlotHoverEnter?.Invoke(index, item);
        }
        ...
        public int GetHoveredIndex() => hoveredSlotIndex;
    }
}