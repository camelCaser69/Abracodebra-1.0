 ...
 public class GameUIManager : MonoBehaviour {
  ...
  void HandleGeneRemovedFromEditor(GeneBase gene, int slotIndex, string slotType) {
   if (RunManager.Instance != null && RunManager.Instance.CurrentState != RunState.Planning) {
    Debug.Log("[GameUIManager] Cannot remove genes outside Planning phase.");
    return;
   }
  ...
  void HandleRunStateChanged(RunState newState) {
   ...
   // Task 8.1: Lock/unlock seed editor based on phase
   bool isPlanning = (newState == RunState.Planning);
   seedEditorController.SetInteractable(isPlanning);
