...
public class PlantNodeExecutor
{
    ...
    public float PoopEnergyBonus { get; set; }
    ...
    public void ExecuteMatureCycleTick()
    {
        ...
        // First pass: Calculate costs and passive buffs
        ...
        // Check for poop *before* spending energy
    ...
    {
        // This reflection is a bit of a workaround to access a private field. A better long-term solution
        // would be to expose the prefab via a public getter in PlantCellManager.
    ...
    public void ApplyScentDataToObject(GameObject targetObject, Dictionary<ScentDefinition, float> scentRadiusBonuses, Dictionary<ScentDefinition, float> scentStrengthBonuses)
    {
        ...
        if (strongestScentDef != null)
        {
            ...
            {
                // Ensure we don't stack particle effects
    ...
    private void CheckForPoopAndAbsorb()
    {
        ...
        // Discretize the radius by rounding to the nearest whole number.
        float discreteRadius = Mathf.RoundToInt(PoopDetectionRadius);
        if (discreteRadius <= 0f) return;

        Collider2D[] colliders = Physics2D.OverlapCircleAll(plant.transform.position, discreteRadius);
